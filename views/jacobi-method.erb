"=========== Meta ============
"StrID : 189
"Title : Jacobi and Gauss-Seidel methods for solving  systems of linear equations
"Slug  : jacobi-method
"Cats  : Sin categoría
"Tags  : 
"=============================
"EditType   : post
"EditFormat : Markdown
"TextAttach : wpid1438-wpid-vimpress_53782029_mkd.txt
"========== Content ==========
[php]
...
[php]section("Jacobi method");[/php]

This was the first iterative method to solve systems of linear equations and was devised by <a href="http://en.wikipedia.org/wiki/Carl_Gustav_Jakob_Jacobi" target="_blank">Carl Gustav Jacob Jacobi</a>(1804-1851). Lets define the system of linear equations as:

[php]figure('jacobi_1', 'Equation system', 'jacobi_1', 'png', 'EN');[/php]

For each row <em>i</em> isolate the $$x\_{i}$$ variable.

[php]figure('jacobi_2', 'Isolate variables', 'jacobi_2', 'png', 'EN');[/php]
...
[php]section("Stop condition");[/php]

You can stop the iterations when $$ |x\_{n+1} - x\_{n}| \leq \epsilon$$ for all $$x\_{i}$$, $$n$$ is the number of iterations.

[php]section("Convergence condition");[/php]

Only if  the coefficient matrix is <a href="http://en.wikipedia.org/wiki/Diagonally_dominant_matrix" target="_target">diagonally dominant</a> the method can guarantee a convergence to the solution. Sometimes the convergence can be achieved even if the conditions are not satisfied, but there is no guarantee.

[php]section("Implementation in Java");[/php]
...
[php]section("Gauss-Seidel Method");[/php]
